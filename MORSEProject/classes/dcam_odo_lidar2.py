...
from classes.orientation import Madgwick
...
class DepthCamera:
    ...
    def get_data(self):
        ...
        :return:
            ...
            odo_x(float): x coordinate of the sensor, in sensor coordinate, in meter
            odo_y(float): y coordinate of the sensor, in sensor coordinate, in meter
        ...
        _, _, _,yaw, pitch, roll = self.odometry_data.get_data()

        # yaw = self.odometry_data.normalise(yaw)
        yaw = np.deg2rad(self.madgwick.yaw())
        pitch = self.odometry_data.normalise(pitch)
        roll = self.odometry_data.normalise(roll)

        yaw = round(yaw, 1)
        pitch = round(pitch,1)
        roll = round(roll,1)
        return points, yaw, pitch, roll
    ...
    def points_cloud_update(self, rgb_image):
        ...
        :param rgb_image: image from which vector of colors to paint scatter plot will be extracted
        """
        # Here we can determine for how long simulation has to collect points to plot them on map

        points, yaw, pitch, roll = self.get_data()
        ...
        for i in range(0, len(points) - 12, 12):
            ...
            # data from odometry is included, translating
            xp = round(x1p + self.robot_pos[0], 1)
            yp = round(y1p + self.robot_pos[1], 1)
            zp = round(z1p + self.robot_pos[2], 1)
        ...
        for i in range(len(data_lidar_list) - 1):  # loop for each beam in lidar sensor
            ...
            if lidar_x != 0 and lidar_y != 0:
                ...
                x_l = round(self.robot_pos[0] + x1, 1)
                y_l = round(self.robot_pos[1] + y1, 1)
                z_l = round(self.robot_pos[2] + z1, 1)
        ...
        try:
            ...
            self.ax.scatter(self.robot_pos[0], self.robot_pos[1], self.robot_pos[2], c='r', marker='o')
            self.ax2.plot(x, y, "b.")
            self.ax2.plot(self.robot_pos[0], self.robot_pos[1], "r.")
